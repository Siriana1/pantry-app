...
import kotlinx.android.synthetic.main.add_item.rvItemsList
...
class Baking : AppCompatActivity() {

    companion object{
        var newNameBaking = "default" //this changes if the name of the category is updated
        var qtyUpdateType2 = ""
    }

    var barcode = "" //to hold the barcode number if customer scans an item
    var from = "" //to record which page the intent came from
    val catCode = "2" //the category code
...
//------------- START OF ONCREATE ---------------------------------------------------------------------------------
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if(category == "chosen"){ //if user is adding a new item by barcode
            enterDetailsExceptBarcode()
        }

        //if from is "add_item", show the enterDetailsExceptBarcode dialog
        if(from == "add_item"){
        ...
        if(from != "from_pantry" && from != "add_item"){
        ...
        homeFromBaking.setOnClickListener {
        ...
        //on click event for the tvCatNameBaking (category name inside the category) button
        tvCatNameBaking.setOnClickListener { view ->
        ...
        btnSubmitCatName.setOnClickListener { view ->
            newNameBaking = etEditCatName.text.toString()
            tvCatNameBaking.text = "$newNameBaking"
            val intent = Intent(this, Pantry::class.java)
            intent.putExtra("NEW_CAT_NAME", newNameBaking)
            intent.putExtra("FROM", "new_category_name")
            startActivity(intent)
            finish()
        }
        //on click event for the submit (Add Item) button - to add a new item by barcode
        btnAddWithScanner.setOnClickListener {
            addRecordByBarcode()
        }
    ...
    private fun removeItemByBarcode() {

            from2 = "2"  // set to the category code so db knows which table to update
            if (barcode != "0"){  // if item was added by the scanner or the barcode was manually added
                ...
                val databaseHandler = DatabaseHandler(this) // create an instance of DatabaseHandler
    ...
    // Displays the enter details without barcode dialog
    ...
    fun enterDetailsExceptBarcode(){
        ...
        // as the item's barcode number does not exist in the db, prompt the user to enter the item's details
            ...
            addItemDialog.btnAdd.setOnClickListener {
                ...
                from2 = "2" //tell db which table to update
    ...
    fun setupListofDataIntoRecyclerView() {
        ...
        from2 = "2" // tell ItemAdapter which page to set up
        if (getItemsBaking().size > 0) { //check there are some items in the Baking table

            rvItemsListBaking.visibility = View.VISIBLE
            tvNoRecordsAvailableBaking.visibility = View.GONE

            // Set the LayoutManager that this RecyclerView will use.
            rvItemsListBaking.layoutManager = LinearLayoutManager(this)
            // Adapter class is initialized and list is passed in the param.
            val itemAdapter = ItemAdapter(this, getItemsList())
            // adapter instance is set to the recyclerview to inflate the items.
            rvItemsListBaking.adapter = itemAdapter
        } else { //if there are no items in the db, display the 'no items available' message

            rvItemsListBaking.visibility = View.GONE
            tvNoRecordsAvailableBaking.visibility = View.VISIBLE
        }
    }
    ...
    //Retrieve Items from the baking table in the database
    fun getItemsBaking(): ArrayList<Item> {
    ...
    fun addRecord(view: View) {
        from2 = "2"  // 2 is the code for baking
        val catCode = "2"
    ...
    fun updateRecordDialog(item: Item) {
        ...
        updateDialog.tvUpdate.setOnClickListener(View.OnClickListener {
            ...
            from2 = "2" //tell the db which table to update
            val databaseHandler: DatabaseHandler = DatabaseHandler(this) //make an instance of DatabaseHandler
    ...
    // Method is used to update the quantity of an item or delete it
    ...
    fun addNewItemDialog() {
        ...
        addItemDialog.btnAdd.setOnClickListener {
            ...
            val bakingItems = getItemsBaking()
            var duplicateItemFound = false
            for(i in bakingItems){
    ...
    fun deleteRecordAlertDialog(item: Item) {
        ...
        builder.setMessage("Are you sure you want to delete BAKING item ${item.name}?")
        builder.setIcon(android.R.drawable.ic_dialog_alert)

        //performing positive action
        builder.setPositiveButton("Yes") { dialogInterface, which ->

            from2 = "2"  //tells db which table item is to be deleted from
            //creating the instance of DatabaseHandler class
            val databaseHandler: DatabaseHandler = DatabaseHandler(this)
